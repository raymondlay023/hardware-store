...
<div {{ $attributes->except(['wire:model', 'wire:model.live', 'wire:model.defer'])->merge(['class' => '']) }}>
    ...
    <select 
        ...
        {{ $attributes->except(['class', 'label', 'name', 'required', 'icon', 'error', 'helpText'])->merge([
            'class' => 'w-full px-3 sm:px-4 py-2 sm:py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent shadow-sm text-sm font-medium transition-all duration-200 ' . ($error ? 'border-red-300 bg-red-50' : 'hover:border-gray-400')
        ]) }}
    >
        {{ $slot }}
    </select>
