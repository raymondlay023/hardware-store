<div>
    <form wire:submit="save" class="space-y-5">
        ...
        <div>
            ...
            @error('low_stock_threshold') <span class="text-red-600 text-sm mt-1 block"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</span> @enderror
        </div>
        ...
        <div>
            ...
            <input 
                type="number" 
                wire:model="critical_stock_threshold"
                placeholder="5"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm">
            <p class="text-xs text-gray-500 mt-1">Urgent alert when stock falls below this level</p>
            @error('critical_stock_threshold') <span class="text-red-600 text-sm mt-1 block"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</span> @enderror
        </div>

        <!-- Auto Reorder Settings -->
        <div class="border-t pt-5">
            <h3 class="text-lg font-semibold text-gray-900 mb-4"><i class="fas fa-cogs mr-2 text-blue-600"></i>Auto-Reorder Settings</h3>
            
            <div class="mb-4">
                <label class="flex items-center gap-3">
                    <input type="checkbox" wire:model.live="auto_reorder_enabled" class="w-4 h-4 text-blue-600 rounded border-gray-300">
                    <span class="text-sm font-medium text-gray-700">Enable auto-reorder when stock is low</span>
                </label>
                <p class="text-xs text-gray-500 mt-2">When enabled, a purchase order will be automatically created when stock falls below the low threshold</p>
            </div>
            ...
            @if($auto_reorder_enabled)
                <div>
                    ...
                    <input 
                        type="number" 
                        wire:model="reorder_quantity"
                        placeholder="e.g., 50"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm">
                    <p class="text-xs text-gray-500 mt-1">Quantity to order when auto-reorder is triggered</p>
                    @error('reorder_quantity') <span class="text-red-600 text-sm mt-1 block"><i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}</span> @enderror
                </div>
            @endif
        </div>
